...
The local D1 runtime is pinned to the stable Miniflare 4 line. Its HTTP client has a scoped `undici` security override; its image dependency is scoped to `sharp` 0.35.4 to address [GHSA-rgj7-g3m4-5g8c](https://github.com/lovell/sharp/security/advisories/GHSA-rgj7-g3m4-5g8c). Esbuild is also pinned. Keep the lockfile and security audit current when changing that runtime. These are development dependencies, not deployed payment SDKs. The production implementation remains Cloudflare Pages Functions with D1 and Stripe's REST APIs.
