  ...
  if (url.pathname === "/__preview-auth" && context.request.method === "POST") {
    ...
    if (!accepted) {
      return previewHeaders(new Response(previewLoginPage({
        invalid: true,
        label: env.PREVIEW_ACCESS_LABEL
...
export async function onRequest(context) {
  ...
  if (hostname === "ocalaaiconsulting.com" || hostname === "www.ocalaaiconsulting.com") {
    return finalizeResponse(
      context.request,
      Response.redirect(`${primaryOrigin}${url.pathname}${url.search}`, 301)
    );
  }
  ...
  const finalResponse = finalizeResponse(context.request, response);
  return env.PREVIEW_ACCESS_TOKEN ? previewHeaders(finalResponse) : finalResponse;
}
