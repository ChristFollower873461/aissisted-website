(function () {
  ...
  async function loadStatus() {
    if (!bookingId && !sessionId) {
      setState("Missing reference", "We could not find a booking reference.", "Return to the booking page and start the reservation again.");
      return;
    }

    try {
      const response = await fetch(`/api/book/status?booking_id=${encodeURIComponent(bookingId)}&session_id=${encodeURIComponent(sessionId)}`, {
        headers: { accept: "application/json" }
      });
      const payload = await response.json();
      if (!response.ok || !payload.ok) throw new Error(payload.error || "Booking status could not be loaded.");
      ...
      switch (payload.confirmationState) {
        ...
        case "awaiting_webhook":
          ...
          return;
        case "expired":
          setState("Expired", "This reservation hold expired before final confirmation.", "No appointment window remains reserved. Return to booking to choose a new slot.");
          return;
        case "failed":
          setState("Payment failed", "The reservation did not complete.", "No booking was confirmed. Choose a new slot or try payment again from booking.");
          return;
        case "manual_review":
          setState("Manual review", "Payment succeeded, but the reservation needs a manual check.", "AIssisted Consulting has the payment record and will follow up directly.");
          return;
        default:
          setState("Processing", "Your reservation is still being finalized.", "If you just completed payment, stay on this page while the confirmation signal arrives.");
          window.setTimeout(loadStatus, 3000);
      }
    } catch (error) {
      setState("Status unavailable", "We could not load the reservation status.", `${error.message} If this persists, email pj@aissistedconsulting.com with your receipt and requested appointment window.`);
    }
  }
